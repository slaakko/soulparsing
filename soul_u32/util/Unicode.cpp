...
namespace soul { namespace unicode {
...
std::string MakeCanonicalPropertyName(const std::string& s)
{
    ...
    return soul::util::ToLower(propertyName);
}
...
NumericTypeTable::NumericTypeTable()
{
    ...
    numericTypes.push_back(NumericType(NumericTypeId::de, "Di", "Digit"));
    numericTypes.push_back(NumericType(NumericTypeId::de, "Nu", "Numeric"));
...
ExtendedCharacterInfo::ExtendedCharacterInfo() : characterName(), canonicalCombiningClass(0), fullUpper(), fullLower(), fullTitle(), fullFolding(), bidiClass(BidiClassId::none), 
    numericType(NumericTypeId::none), numericValue(), bidiMirroringGlyph(0), bidiPairedBracket(0)
{
}
...
std::string SoulU32Root()
{
    ...
    {
        throw UnicodeException("please set 'SOUL_U32_ROOT' environment variable to contain /path/to/soul_u32 directory.");
    }
    return soul_u32_root;
}

std::string SoulU32UcdFilePath()
{
    return (boost::filesystem::path(SoulU32Root()) / boost::filesystem::path("unicode") / boost::filesystem::path("soul_ucd.bin")).generic_string();
}
...
{
    std::string ucdFilePath = SoulU32UcdFilePath();
